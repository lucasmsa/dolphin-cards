...
public interface FlashCardsRepository extends JpaRepository<FlashCard, String> {
    ...
    @Query(value = "SELECT * FROM FLASH_CARDS as fc JOIN DISCIPLINES as d ON fc.discipline_id = d.id JOIN STUDENTS as s ON d.student_id = s.id  where s.id = ?1", 
           countQuery = "SELECT count(*) FROM FLASH_CARDS as fc JOIN DISCIPLINES as d ON fc.discipline_id = d.id JOIN STUDENTS as s ON d.student_id = s.id  where s.id = ?1",
           nativeQuery = true)
